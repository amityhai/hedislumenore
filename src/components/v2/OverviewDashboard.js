...
const STATUS = [
  { key: 'all', label: 'All measures', status: null, tone: 'total', helper: 'Active HEDIS measures' },
...
const OverviewDashboard = ({ token, selectedMonth, onMonthChange, availableMonths, onInvestigate, onOpenProvider, onOpenMember }) => {
  ...
  return (
    <div className="od">
      ...
      {data?.sample && !loading && <div className="od-data-note" role="status"><span>Live workflow unavailable — showing representative data.</span><button onClick={refetch}>Try again</button></div>}
