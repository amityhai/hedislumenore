...
const PerformanceChart = ({ rows, loading, filterLabel, onSelect }) => {
  ...
  return (
    <section className="od-card od-chart-card">
      <div className="od-section-head">
        ...
        <span className="od-context-pill">{filterLabel}</span>
      </div>
  ...
  return (
    <div className="od">
      ...
      <div className="od-analysis-grid">
        <PerformanceChart rows={filtered} loading={loading} filterLabel={activeDef.label} onSelect={(m) => { setSelectedMeasureId(m.measure_id); setWorkTab('Measures'); }} />
        ...
        <div className="od-table-scroll">
          {loading || (workTab === 'Providers' && providersAsync.loading) ? <div className="od-table-loading">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} height={55} radius={8} />)}</div> : searchedRows.length === 0 ? <EmptyState icon="—" title={`No ${workTab.toLowerCase()} found`} hint="Try a different status or search." /> : (
            <table className="od-table">
              {workTab === 'Measures' && <><thead><tr><th>Measure</th><th>Achieved</th><th>Goal</th><th>Gap</th><th>Member gap</th><th>Status</th><th>Actions</th></tr></thead><tbody>{searchedRows.slice(0, 10).map((m) => { const tone = STATUS_TONE[m.kpi_status] || 'below'; const gap = gapOf(m); return <tr key={m.measure_id} className={m.measure_id === selectedMeasureId ? 'is-selected' : ''} onClick={() => setSelectedMeasureId(m.measure_id)}><td><span className="od-entity"><i className={`is-${tone}`}>{shortId(m.measure_id).slice(0, 3)}</i><span><strong>{m.display_name}</strong><small className="mono">{shortId(m.measure_id)}</small></span></span></td><td className="num od-strong">{num(m.rate)}%</td><td className="num">{num(m.goal_50th)}%</td><td className={`num ${gap < 0 ? 'is-negative' : 'is-positive'}`}>{gap >= 0 ? '+' : ''}{gap} pts</td><td><strong className="num">{fmt(openGaps(m))}</strong><small> open</small></td><td><span className={`od-status is-${tone}`}><i />{m.kpi_status}</span></td><td><span className="od-row-actions"><button className="btn btn-assign btn-sm" onClick={(e) => { e.stopPropagation(); setAssignScope({ measure: m, level: 'measure' }); }}>Assign intervention</button><button className="btn btn-primary btn-sm" onClick={(e) => { e.stopPropagation(); onInvestigate?.(m); }}>Open list <Icon name="arrow" size={13} /></button></span></td></tr>; })}</tbody></>}
